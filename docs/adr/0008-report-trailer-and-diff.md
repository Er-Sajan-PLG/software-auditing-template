...
USA already decided Markdown is the only output format (ADR-0004). The
...
HTML comment (`<!-- USA:TRAILER:BEGIN --> … <!-- USA:TRAILER:END -->`):
schema version, timestamp, overall score, per-section scores, severity
totals, and every rule's status/severity/section. `usa diff old.md new.md`
...
1. Rule IDs are pack-author-controlled input to a machine channel, so keys
   ...
   newline, or a fence once corrupted the mapping `usa diff` parses.
...
reports (`usa diff Q2.md Q3.md`) work with zero infrastructure.
...
`schema: usa-report-v1` exists so a future v2 can be detected, not guessed.
